...
class AFFModel(nn.Module):
    ...
    def forward(self, query):
        ...
        rects = rects.view(rects.size(0), -1) # grid in itracker 25*25
        xRect = self.rects_fc(rects) # rects 3 bounding boxes 3*4
...
class AFFDelta(nn.Module):

    def __init__(self):
        super(AFFDelta, self).__init__()
        self.eyeModel = EyeImageModel()
        self.eyesMerge_1 = nn.Sequential(
                         SELayer(256, 16),  
                         nn.Conv2d(256, 64, kernel_size=3, stride=2, padding=1),
    ...
    def forward(self, query, calib):
        ...
        # Cat all
        xq = torch.cat((xEyes, xFace, xRect), 1)
        ...
        # Cat all
        xc = torch.cat((xEyes, xFace, xRect), 1)
...
if __name__ == '__main__':
    ...
    #            "rightEyeImg": torch.zeros(10, 3, 112, 112), "faceGridImg": torch.zeros(10, 25, 25),
