...
class DeltaGazeModel(nn.Module):
    ...
    def __init__(self):
        ...
        '''
        self.regressor = nn.Linear(16+16, 2)
        self.classifier = nn.Linear(16+16, 4)
...
class KanDeltaModel(nn.Module):
    '''
    delta gaze model based on KAN
    
    '''

    def __init__(self):
        super(KanDeltaModel, self).__init__()
        self.eyeModel = EyeImageModel()
        self.faceModel = FaceImageModel()
        self.gridModel = FaceGridModel()
        ...
        self.featureFC = nn.Sequential(
            KAN(layers_hidden = [16*4,16]),
            nn.ReLU(inplace=True),
            )
        '''
        self.featureFC = KAN(layers_hidden = [16*4,16])
        '''
        # Joining query and calib
        self.regressor = KAN(layers_hidden = [16*2,2])
        self.classifier = KAN(layers_hidden = [16*2,4])
